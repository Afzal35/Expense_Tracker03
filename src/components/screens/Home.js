...
const Home = (props) => {
    ...
    useEffect(() => {
        fetch(`https://expense-tracker-1d713-default-rtdb.firebaseio.com/expenses/${username}.json`)
            .then((res) => {
                if (!res.ok) {
    ...
    const sendData = async (expenses) => {
        const res = await fetch(
            `https://expense-tracker-1d713-default-rtdb.firebaseio.com/${username}.json`,
            {
                method: "POST",
    ...
    const editDeleteHandler = (id, isEdit) => {
        ...
        fetch(`https://expense-tracker-1d713-default-rtdb.firebaseio.com/expenses/${username}/${id}.json`, {
            method: "DELETE"
        }).then((res) => {
            if (res.ok) {
                return res.json();
